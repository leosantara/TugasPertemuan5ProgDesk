...
Public Class form1
    ...
    Private Sub DeleteFileToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles DeleteFileToolStripMenuItem.Click
        Dim item As ListViewItem = lvFile.SelectedItems(0)
        Dim filePath As String = item.Tag
        Dim result As DialogResult = MessageBox.Show("Are you sure you want to delete " & filePath & "?", "Delete", MessageBoxButtons.YesNo, MessageBoxIcon.Question)
    ...
    Private Sub RenameToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles RenameToolStripMenuItem.Click
        If lvFile.SelectedItems.Count > 0 Then
            Dim item As ListViewItem = lvFile.SelectedItems(0)
            Dim filePath As String = item.Tag ' mengambil nilai path file dari tag ListViewItem
            Dim fileName As String = IO.Path.GetFileName(filePath)
            Dim result As String = InputBox("Enter new file name:", "Rename", fileName)
            If Not String.IsNullOrEmpty(result) Then
                Try
                    IO.File.Move(filePath, IO.Path.Combine(IO.Path.GetDirectoryName(filePath), result))
                    item.Text = result ' mengubah nilai pada kolom Name ListView
                    item.Tag = IO.Path.Combine(IO.Path.GetDirectoryName(filePath), result) ' mengubah nilai tag ListViewItem
